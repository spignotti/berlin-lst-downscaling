...
# stop VM. Products (QA evidence) live in GCS, not on the VM disk.
#
...
#
# Usage:
#   scripts/run_qa_stage1_vm.sh [branch]
...
WRAP_RUN_ID="qa-stage1-$(date -u +%Y%m%dT%H%M%SZ)"
...
echo "Stage-1 QA | Branch: $BRANCH | Run: $WRAP_RUN_ID"
...
echo "Polling for QA completion ($WRAP_RUN_ID)..."
POLL_FAILURES=0
...
while true; do
  ...
  if [[ -n "$TERMINAL" ]]; then
    if [[ "$TERMINAL" == "0" ]]; then
      echo "  [$(date +%H:%M:%S)] QA completed successfully."
    else
      echo "  [$(date +%H:%M:%S)] QA exited with code $TERMINAL."
    fi
    break
  fi

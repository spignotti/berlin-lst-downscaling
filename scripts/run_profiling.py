...
def run_profiling(cfg: DictConfig) -> int:
    ...
    # Check manifest↔ledger completeness (hard failure on ledger read errors)
    ...
    # Check dynamic coverage against manifest anchors
    log_event(_logger, logging.INFO, "checking_dynamic_coverage")
    coverage = check_dynamic_coverage(
        manifest_uri=cfg.manifest_uri,
        dynamic_root=cfg.dynamic_root,
        inference_root=cfg.inference_root,
    )
    ...
    emit_artifacts(rows, output_root, completeness=completeness, coverage=coverage)

...
"""Landsat ARD pipeline entry point — Hydra-driven, mode=smoke|full.

Usage
-----
    uv run python scripts/run_ard_landsat.py --config-name smoke
    uv run python scripts/run_ard_landsat.py --config-name smoke scene_date=2024-07-15 viz=false
    uv run python scripts/run_ard_landsat.py --config-name full
...
_HYDRA_CONFIG_PATH = "../configs/ard/landsat"
...
def main(cfg: DictConfig) -> int:
    ...
    if cfg.sources != ["landsat-c2-l2"]:
        raise ValueError(f"run_ard_landsat.py requires sources=[landsat-c2-l2], got {cfg.sources}")

    print("=" * 60, flush=True)
    print(f"ARD Pipeline (Landsat) — mode={cfg.mode}", flush=True)
